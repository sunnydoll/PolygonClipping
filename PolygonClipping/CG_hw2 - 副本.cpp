...
void transform(int& x, int& y, int sign, int index, int length, double scal, int rota, int xTran, int yTran);
...
void bresenham(int qx, int qy, int rx, int ry, int sign, int index, double scal, int rota, int xTran, int yTran)
{
	...
	int length_line = 0;
	int dx, dy, D, x, y = 0;
	transform(qx, qy, sign, index, length_line, scal, rota, xTran, yTran);
	transform(rx, ry, sign, index, length_line, scal, rota, xTran, yTran);
...
void transform(int& x, int& y, int sign, int index, int length, double scal, int rota, int xTran, int yTran)
{
	...
	switch (sign)
	{
		...
		case 1:
			...
			break;
		case 2:	
			x = tempY * cosValue - tempX * sinValue + xTran;
			y = tempY * sinValue + tempX * cosValue + yTran;
		...
		case 3:
			x = (-tempY) * cosValue - tempX * sinValue + xTran;
			y = (-tempY) * sinValue + tempX * cosValue + yTran;
			/*xOpPath[index][length] = -(tempX * cosValue - tempY * sinValue) + xTran;
			yOpPath[index][length] = tempX * sinValue + tempY * cosValue + yTran;*/
			/*xOpPath[index][length] = -y;
			yOpPath[index][length] = x;*/
			break;
		/*case 4:
